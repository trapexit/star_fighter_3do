...
char	backdrop_file [48];
	...
	if (backdrop == NULL)
		backdrop = (char*) LoadImage (backdrop_file, NULL, NULL, screen);					// Load in backdrop & allocate buffer
	else
		backdrop = (char*) LoadImage (backdrop_file, backdrop, NULL, screen);				// Load in backdrop & allocate buffer
}

/**************************************/

//void	unload_backdrop (void)

// Purpose : Unloads a backdrop from memory
// Accepts : Nothing
// Returns : Nothing
