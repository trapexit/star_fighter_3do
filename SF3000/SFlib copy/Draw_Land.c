	...
	&& ( camera_y_rotation > ((256*3)+128) || camera_y_rotation < ((256*3)-128) ) )
{
		...
		size_x = ( node_data.x_pos * pex_table [ (node_data.y_pos)>>5 ])>>16 ;
		size_y = ( node_data.z_pos * pex_table [ (node_data.y_pos)>>5 ])>>16 ;
...
if (ata_selected != 0)
{
	...
	if (players_ship->last_fire_request == WEAPON_BEAM_LASER || which_graphics_set == SPACE_GRAPHICS )
	{
