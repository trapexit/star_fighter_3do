...
perspective_table_high_res
	DCD pex_table_near

high_res_pex_is_on

	ldr r0,graphic_rotated_coords_1
	ldr r1,perspective_table_high_res
	ldr r2,graphic_screen_coords_1
	ldr r3,node_counter_2
	
	bl do_perspective_high_res
...
yes_i_am_a_atg_lock_on_a_truck_or_tank
	DCD 0
...
normal_plot_ship	
	...
	mov r0,#0
	str r0,yes_i_am_a_atg_lock_on_a_truck_or_tank

	ldr r0,atg_selected
	cmp r0,#0
	beq no_atg_lock_on_trucks_n_tanks

	ldr r0,ship_to_plot
	ldr r0,[r0,#ship_type]
	mov r0,r0, asr #4
	cmp r0,#CAR
	bne no_atg_lock_on_trucks_n_tanks

	mov r0,#1
	str r0,yes_i_am_a_atg_lock_on_a_truck_or_tank
	
	b yes_atg_lock_on_trucks_n_tanks

no_atg_lock_on_trucks_n_tanks

	ldr r0,ata_selected
	cmp r0,#0
	ldmeqfd sp!,{pc}

	;ata don't like trucks
	ldr r0,ship_to_plot
	ldr r0,[r0,#ship_type]
	mov r0,r0, asr #4
	cmp r0,#CAR
...
yes_atg_lock_on_trucks_n_tanks
...
plot_ship
	...
	ldr r3,[r0,#ship_type]		;get type
	
	;Is it a explosion section
	cmp r3,#255
	
	;Last minute fix the missile lock !!!!!!!!!!!!!!!!!!
	movge r4,#0
	movlt r4,#1
	str r4,i_am_a_ship	
	...
	;How like far is it allowed behind the camera
...
plot_static_graphic
	...
	;Last minute fix the missile lock !!!!!!!!!!!!!!!!!!
	cmp r3,#60			;check type - cloud or woty 
	movge r5,#0
	movlt r5,#1
	str r5,i_am_a_object
...
clip_dist_ok_4
	...
	ldr r9,perspective_table_high_res
...
;Is the 1st coord in front of the camera - if true calc screen pos
	cmp r5,#0
	ble fail_pex_1_high_res ; Skip code if the point is behind the view point
	bic r5,r5,#31
...
fail_pex_1_high_res
	...
	subs r3,r3,#1
	bmi perspective_end_high_res

;2nd coord
	cmp r8,#0
	ble fail_pex_2_high_res
	bic r8,r8,#31
	ldr r8,[r1,r8, asr #3]
	mul r13,r7,r8
	mov r7,r13, asr #16
...
draw_collision_box_lines_loop
	...
	adrl r5,yes_i_am_a_atg_lock_on_a_truck_or_tank
	ldr r5,[r5,#0]
	cmp r5,#0
	movne r4,#cyan
