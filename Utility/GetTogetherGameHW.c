...
// --------------------------------
	...
	logit ("\n\nEXPORT Conversion of Game cels [WIDTH + HEIGHT INCLUDED]\n");
...
// --------------------
			...
			if (finished_master == 0)
				...
				if (file_in != NULL)
					...
					// GET WIDTH & HEIGHT DATA [FOR CONVERSIONS]
					
					fseek (file_in, 72, SEEK_SET);
					fread (&wordin, sizeof(long), 1, file_in);			// Get width of sprite
					...
					fseek (file_in, 76, SEEK_SET);
					fread (&wordin, sizeof(long), 1, file_in);			// Get height of sprite
					...
					if ((word & (1<<23)) != 0 || master_title == 'p')
						...
						// Yes - find out PLUT data position and length, then copy
						
						fseek (file_in, 84, SEEK_SET);
						fread (&word, sizeof(long), 1, file_in);			// Get Offset to PLUT
						word+=80;											// Start of PLUT
						word+=12;											// No. of entries location
					...
					if (master_title [1] != 'p')							// Don't put in PREAMBLE OR SOURCEDATA if  just doing PLUT
						{
						fread (&word, sizeof(long), 1, file_in);			// Copy Preamble0
						buildfile[buildptr++] = *(wordin+0);
						buildfile[buildptr++] = *(wordin+1);
						buildfile[buildptr++] = *(wordin+2);
						buildfile[buildptr++] = *(wordin+3);			
						
						fseek (file_in, 92, SEEK_SET);						// Copy over sprite data		
			...
			sprintf(&filename_out,"%sExport:Game.%s", hdisk, gamemaster_title);
